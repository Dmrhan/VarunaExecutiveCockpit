...
export function QuotePoolAnalysis({ data, countData, onStatusSelect, selectedStatus }: QuotePoolAnalysisProps) {
    ...
    const poolData = useMemo(() => {
        ...
        // Define colors and order for specific statuses if needed, otherwise default
        const statusConfig: Record<string, { color: string; bg: string }> = {
            'Draft': { color: 'bg-slate-400', bg: 'bg-slate-100 dark:bg-slate-800' },
            'Review': { color: 'bg-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-900/20' },
            'Sent': { color: 'bg-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' },
            'Accepted': { color: 'bg-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
            'Approved': { color: 'bg-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
            'Rejected': { color: 'bg-rose-500', bg: 'bg-rose-50 dark:bg-rose-900/20' },
            'Denied': { color: 'bg-rose-600', bg: 'bg-rose-50 dark:bg-rose-900/20' },
        };
    ...
    return (
        <div className="bg-white/40 dark:bg-slate-700/40 backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm flex flex-col h-full min-h-[400px]">
            ...
            <div className="flex-1 flex items-end justify-between gap-2 sm:gap-4 px-2 overflow-x-auto pb-2">
                {poolData.map((pool) => {
                    ...
                    return (
                        <div
                            ...
                            className={cn(
                                "flex flex-col items-center gap-3 w-16 sm:w-20 group cursor-pointer transition-all duration-300",
                        ...
                        >
                            ...
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -mt-8 bg-slate-900 text-white text-[10px] py-1 px-2 rounded pointer-events-none z-10 whitespace-nowrap">
                                {formatCurrency(pool.revenue)} ({pool.count} Adet)
                            </div>
                            ...
                            <div className="text-center">
                                <div className="text-[10px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-tight truncate max-w-full">
                                    {t(`quotes.status.${pool.name}`, { defaultValue: pool.name })}
                                </div>
                                <div className="text-[9px] text-slate-400 font-medium mt-0.5">
                                    {pool.count} {t('quotes.units.count', { defaultValue: 'Adet' })}
                                </div>
                            </div>
                        </div>
                    );
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 flex items-center gap-2 text-slate-400">
                <Info size={14} />
                <span className="text-[10px]">{t('quotes.poolAnalysis.heightParams')}</span>
            </div>
        </div>
    );
}
