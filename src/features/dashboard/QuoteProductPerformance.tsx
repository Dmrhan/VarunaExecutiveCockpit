...
import type { ProductGroup, Quote } from '../../types/crm';
import { PRODUCT_COLORS } from '../../data/mockData';
import * as XLSX from 'xlsx';
import jsPDF from 'jspdf';
import 'jspdf-autotable';

interface QuoteProductPerformanceProps {
    quotes?: Quote[];
}

export function QuoteProductPerformance({ quotes: propQuotes }: QuoteProductPerformanceProps) {
    const { t, i18n } = useTranslation();
    const { quotes: contextQuotes, users } = useData();
    const quotes = propQuotes || contextQuotes;
    ...
    const filteredQuotes = useMemo(() => {
        if (!selectedStatus) return productQuotes;
        return productQuotes.filter(q => q.status === selectedStatus);
    }, [productQuotes, selectedStatus]);

    // Analytics Data
    const analyticsData = useMemo(() => {
        if (!productQuotes.length) return { statusData: [], trendData: [] };

        // Status Distribution (Revenue Based)
        const byStatus = productQuotes.reduce((acc, quote) => {
            if (!acc[quote.status]) {
                acc[quote.status] = { value: 0, count: 0 };
            }
            acc[quote.status].value += quote.amount;
        ...
        productQuotes.forEach(quote => {
            ...
            if (monthEntry) {
                monthEntry.value += quote.amount;
            }
        });

        return { statusData, trendData: last6Months };
    }, [productQuotes, i18n.language]);

    const COLORS = ['#10b981', '#6366f1', '#f43f5e', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4'];

    const exportToExcel = () => {
        try {
            const data = filteredQuotes.map(q => ({
                [t('opportunities.customer')]: q.customerName,
                [t('productPerformance.seller')]: users.find(u => u.id === q.salesRepId)?.name || q.salesRepId,
                [t('quotes.table.name')]: q.title,
                [t('productPerformance.createdAt')]: new Date(q.createdAt).toLocaleDateString(i18n.language),
                [t('quotes.table.amount')]: q.amount,
                [t('quotes.table.status')]: t(`quotes.status.${q.status}`, { defaultValue: q.status }),
                [t('performance.validUntil')]: new Date(q.validUntil).toLocaleDateString(i18n.language)
            }));

            const ws = XLSX.utils.json_to_sheet(data);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, selectedProduct || 'Quotes');
            XLSX.writeFile(wb, `${selectedProduct}_Quotes_${new Date().toISOString().split('T')[0]}.xlsx`);
        } catch (error) {
    ...
    const exportToPDF = () => {
        try {
            ...
            const tableData = filteredQuotes.map(q => [
                q.customerName,
                users.find(u => u.id === q.salesRepId)?.name || q.salesRepId,
                q.title,
                new Date(q.createdAt).toLocaleDateString('tr-TR'),
                `$${q.amount.toLocaleString()}`,
                t(`quotes.status.${q.status}`, { defaultValue: q.status }),
            ...
            doc.autoTable({
                head: [[
                    ...
                    t('performance.validUntil')
                ]],
                body: tableData,
                theme: 'grid',
                headStyles: { fillColor: [99, 102, 241] } // Indigo color for Quotes
            });

            doc.save(`${selectedProduct}_Quotes_${new Date().toISOString().split('T')[0]}.pdf`);
        } catch (error) {
                                            ...
                                            <ResponsiveContainer width="100%" height="100%">
                                                <AreaChart data={analyticsData.trendData}>
                                                    ...
                                                    <Tooltip
                                                        ...
                                                        itemStyle={{ color: '#fff' }}
                                                        formatter={(value: any) => [`$${value?.toLocaleString() ?? 0}`, t('quotes.table.amount')]}
                                                    />
                                    ...
                                    <table className="w-full text-left border-separate border-spacing-y-2">
                                        <thead className="sticky top-0 bg-transparent z-10">
                                            <tr className="text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">
                                                ...
                                                <th className="px-4 pb-4">{t('quotes.table.name')}</th>
                                                <th className="px-4 pb-4 text-center">{t('quotes.table.status')}</th>
                                                <th className="px-4 pb-4 text-right">{t('quotes.table.amount')}</th>
                                                <th className="px-4 pb-4 text-right">{t('performance.validUntil', { defaultValue: 'Geçerlilik Tarihi' })}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {filteredQuotes.map((quote) => {
                                                const rep = users.find(u => u.id === quote.salesRepId);
                                                return (
                                                    <tr key={quote.id} className="group transition-all hover:translate-x-1">
                                                        <td className="bg-white dark:bg-white/5 p-4 first:rounded-l-2xl border-y border-l border-slate-200 dark:border-white/10 shadow-sm">
                                                            <span className="text-xs md:text-sm font-semibold text-slate-900 dark:text-white leading-tight block">{quote.customerName}</span>
                                                            <span className="text-[10px] text-slate-400">{new Date(quote.createdAt).toLocaleDateString(i18n.language)}</span>
                                                        </td>
                                                        <td className="bg-white dark:bg-white/5 p-4 border-y border-slate-200 dark:border-white/10 shadow-sm">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span className="text-xs md:text-sm text-slate-600 dark:text-slate-300">{rep?.name || quote.salesRepId}</span>
                                                            </div>
                                                        </td>
                                                        <td className="bg-white dark:bg-white/5 p-4 border-y border-slate-200 dark:border-white/10 shadow-sm">
                                                            <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{quote.title}</span>
                                                        </td>
                                                        ...
                                                        <td className="bg-white dark:bg-white/5 p-4 border-y border-slate-200 dark:border-white/10 text-right shadow-sm">
                                                            <span className="text-xs md:text-sm font-bold text-emerald-600 dark:text-emerald-400">${(quote.amount / 1000).toFixed(0)}k</span>
                                                        </td>
                                                        <td className="bg-white dark:bg-white/5 p-4 last:rounded-r-2xl border-y border-r border-slate-200 dark:border-white/10 text-right shadow-sm">
                                                            <span className="text-xs text-slate-500 font-mono">
                                                                {new Date(quote.validUntil).toLocaleDateString(i18n.language)}
                                                            </span>
                                                        </td>
                                                    </tr>
                                                );
