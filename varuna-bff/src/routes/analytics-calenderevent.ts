...
router.get('/', (_req: Request, res: Response) => {
    try {
        ...
        // 1. Activity Count by Owner
        const activityCountByOwner = db.prepare(`
        ...
        const activityCountByType = db.prepare(`
        ...
        const meetingsPerAccount = db.prepare(`
        ...
        const newAccountActivitiesCount = (db.prepare(`
            SELECT COUNT(*) as n FROM CalenderEvent WHERE IsNewAccount = 1
        `).get() as { n: number }).n || 0;

        // 5. Activity Completion Rate
        let completionRate = 0;
        const totalActivities = (db.prepare(`SELECT COUNT(*) as n FROM CalenderEvent`).get() as { n: number }).n || 0;
        if (totalActivities > 0) {
            const completedActivities = (db.prepare(`SELECT COUNT(*) as n FROM CalenderEvent WHERE Status = 2`).get() as { n: number }).n || 0; // Assuming 2 is Completed
            completionRate = completedActivities / totalActivities;
        }

        // 6. Activity Duration (Average Minutes) per Type
        const avgDurationPerType = db.prepare(`
            SELECT 
                TypeTr, 
                AVG((julianday(FinishDate) - julianday(StartDate)) * 24 * 60) as AvgDurationMinutes
            FROM CalenderEvent
        ...
        // 7. Rep Productivity Density (Activities per Month)
        const productivityDensity = db.prepare(`
            SELECT 
                c.OwnerId,
                p.PersonNameSurname as FullName,
                substr(c.StartDate, 1, 7) as ActivityMonth,
                COUNT(c.Id) as MonthlyActivityCount,
                SUM(CASE WHEN c.Status = 2 THEN 1 ELSE 0 END) as CompletedActivities,
                CAST(SUM(CASE WHEN c.Status = 2 THEN 1 ELSE 0 END) AS REAL) / NULLIF(COUNT(c.Id), 0) as CompletionRate
            FROM CalenderEvent c
            LEFT JOIN Person p ON c.OwnerId = p.Id
            WHERE c.StartDate IS NOT NULL
            GROUP BY c.OwnerId, p.PersonNameSurname, substr(c.StartDate, 1, 7)
            ORDER BY p.PersonNameSurname, ActivityMonth DESC
        `).all();

        // 8. Repeat vs Unique Activity Mix
        const repeatUniqueMix = db.prepare(`
            SELECT 
        ...
        const meetingRevenueCorrelation = db.prepare(`
            SELECT 
                a.Id as AccountId,
