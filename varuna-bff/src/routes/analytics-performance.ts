...
router.get('/daily', (req: Request, res: Response) => {
    try {
        ...
        const monthlyTarget = Number(req.query.monthlyTarget) || 50000000;
        const yearlyTarget = Number(req.query.yearlyTarget) || 600000000;
        ...
        if (req.query.teamId) {
            ...
            const teamFilter = ` AND SalesRepresentativeId IN (SELECT PersonId FROM TeamMember WHERE TeamId IN (${placeholders}))`;
            const orderTeamFilter = ` AND ProposalOwnerId IN (SELECT PersonId FROM TeamMember WHERE TeamId IN (${placeholders}))`;

            ownerWhere += teamFilter;
            orderOwner += orderTeamFilter;
        ...
        const MonthlyContractAchievementRate = monthlyTarget > 0 ? (monthlyContractRow.amt / monthlyTarget) * 100 : 0;
        const PipelineImpactMonthlyRate = monthlyTarget > 0 ? ((monthlyContractRow.amt + openContractRow.amt) / monthlyTarget) * 100 : 0;
        ...
        const RevenueGap = Math.max(0, yearlyTarget - ytdInvoiceRow.amt);
        ...
        res.json({
            WeeklyContractCount: weeklyContractRow.cnt,
            WeeklyContractAmount: weeklyContractRow.amt,
            MonthlyContractCount: monthlyContractRow.cnt,
            MonthlyContractAmount: monthlyContractRow.amt,
            OpenContractCount: openContractRow.cnt,
            OpenContractPotentialAmount: openContractRow.amt,
            MonthlyContractTarget: monthlyTarget,
            MonthlyContractAchievementRate,
            PipelineImpactMonthlyRate,
            PipelineImpactYearlyRate,
            TodayInvoiceCount: todayInvoiceRow.cnt,
            TodayInvoiceAmount: todayInvoiceRow.amt,
            WeeklyInvoiceCount: weeklyInvoiceRow.cnt,
            WeeklyInvoiceAmount: weeklyInvoiceRow.amt,
            MonthlyInvoiceCount: monthlyInvoiceRow.cnt,
            MonthlyInvoiceAmount: monthlyInvoiceRow.amt,
            YTDInvoiceAmount: ytdInvoiceRow.amt,
            MonthlyInvoiceTarget: monthlyTarget,
            YearlyInvoiceTarget: yearlyTarget,
            MonthlyAchievementRate,
            YearlyAchievementRate,
            RevenueGap,
            TodayDueReceivable: todayDueRow.amt,
            TodayCollectedAmount: todayCollectedRow.amt,
            OpenReceivableRisk: openReceivableRiskRow.amt,
            CollectionRatio,
            Deltas: {
...
router.get('/monthly', (req: Request, res: Response) => {
    try {
        ...
        const periodMap: Record<string, { PeriodKey: string; ContractAmount: number; InvoiceAmount: number; CollectionAmount: number; TargetAmount: number }> = {};
        for (const row of contractTrends) {
            periodMap[row.PeriodKey] = periodMap[row.PeriodKey] || { PeriodKey: row.PeriodKey, ContractAmount: 0, InvoiceAmount: 0, CollectionAmount: 0, TargetAmount: monthlyTarget };
            periodMap[row.PeriodKey].ContractAmount = row.ContractAmount;
        }
        for (const row of invoiceTrends) {
            periodMap[row.PeriodKey] = periodMap[row.PeriodKey] || { PeriodKey: row.PeriodKey, ContractAmount: 0, InvoiceAmount: 0, CollectionAmount: 0, TargetAmount: monthlyTarget };
            periodMap[row.PeriodKey].InvoiceAmount = row.InvoiceAmount;
        }
        for (const row of collectionTrends) {
            periodMap[row.PeriodKey] = periodMap[row.PeriodKey] || { PeriodKey: row.PeriodKey, ContractAmount: 0, InvoiceAmount: 0, CollectionAmount: 0, TargetAmount: monthlyTarget };
            periodMap[row.PeriodKey].CollectionAmount = row.CollectionAmount;
        }
...
router.get('/burnup', (req: Request, res: Response) => {
    try {
        ...
        const payDateSubstr = db.driver === 'mssql' ? 'LEFT(cpp.PaymentDate, 10)' : 'substr(cpp.PaymentDate, 1, 10)';
        const collectionDaily = db.query(`SELECT ${payDateSubstr} as date, COALESCE(SUM(cpp.Price_Amount), 0) as amt FROM ContractPaymentPlans cpp ${contractJoin} WHERE cpp.HasBeenCollected = 1 AND cpp.PaymentDate >= ? AND cpp.PaymentDate <= ? GROUP BY ${payDateSubstr} ORDER BY date ASC`, [monthStart, asOfDate, ...joinParams]) as { date: string; amt: number }[];

        const dayMap: Record<string, { date: string; contractAmt: number; invoiceAmt: number; collectionAmt: number }> = {};
        const startD = new Date(monthStart);
        const endD = new Date(asOfDate);
        for (let d = new Date(startD); d <= endD; d.setDate(d.getDate() + 1)) {
            const key = d.toISOString().split('T')[0];
            dayMap[key] = { date: key, contractAmt: 0, invoiceAmt: 0, collectionAmt: 0 };
        }
        ...
        for (const row of collectionDaily) { if (dayMap[row.date]) dayMap[row.date].collectionAmt = row.amt; }

        let cumContract = 0; let cumInvoice = 0; let cumCollection = 0;
        const burnup = Object.values(dayMap).sort((a, b) => a.date.localeCompare(b.date)).map(day => {
            cumContract += day.contractAmt;
            cumInvoice += day.invoiceAmt;
            cumCollection += day.collectionAmt;
            return { date: day.date, cumulativeContract: cumContract, cumulativeInvoice: cumInvoice, cumulativeCollection: cumCollection };
        });
